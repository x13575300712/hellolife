...
var DataProcess = {
	inputData : function(nodes,startId) {
		...
		var distance = graphlib.alg.dijkstra(g, startId)
	...
	dealNodeMytest : function(g, node) {
		if (node.nextNode) {
			for (var i = 0; i < node.nextNode.length; i++) {
				this.setEdge(g, node.id, node.nextNode[i].nodeid)
			}
		}
	},
...
var DataDraw = {
	...
	draw : function(result) {
		...
		//节点定位
		this.computeXY(nodes,result.startId)
	...
	connectEndpointFromId : function(from, to,oid) {
		jsPlumb.connect({
			uuids : [ from, to ],
		    id  : oid
		})
	...
	computeXY : function(nodes,startId) {
		var matrix = DataProcess.inputData(nodes,startId)
		var base = {
			topBase : 50,
			topStep : 150,
			leftBase : 150,
		...
		for (var i = 0; i < matrix.length; i++) {
			for (var j = 0; j < matrix[i].length; j++) {
				...
				switch((i+ad)%4){
					case 0 : 
						dest.top = dest.top || base.topBase + (i+ad)/4 * base.topStep;
						dest.left = dest.left || base.leftBase + j * base.leftStep;
						break;
					case 1 :
						dest.top = dest.top || base.topBase + ((i-1+ad)/4) * base.topStep;
						dest.left = dest.left || base.leftBase + (j+1) * base.leftStep;
						break;
					case 2 :
						dest.top = dest.top || base.topBase + ((i-2+ad)/4) * base.topStep;
						dest.left = dest.left || base.leftBase + (j+2) * base.leftStep;
						break;
					case 3 :
						dest.top = dest.top || base.topBase + ((i-3+ad)/4) * base.topStep;
						dest.left = dest.left || base.leftBase + (j+3) * base.leftStep;
						break;
				}
			}
			ad = ad +matrix[i].length-1;
		}
	},
	...
	addEndpointOfMytest : function(node) {
		addDraggable(node.id)
		setEnterPoint(node.id)
		setExitPoint(node.id)
	},
	...
	connectEndpointOfMytest : function(node) {
		if(node.nextNode){
			for(var i = 0 ;i<node.nextNode.length;i++){
				this.connectEndpointFromId(node.id + '-out', node.nextNode[i].nodeid + '-in',node.nextNode[i].id)
			}
		}
	},
	...
	connectEndpointOfWorkTime : function(node) {
		this.connectEndpoint(node.id + '-onWorkTime-out', node.data.onWorkNode
				+ '-in')
		this.connectEndpoint(node.id + '-offWorkTime-out',
				node.data.offWorkNode + '-in')
	},
	...
	connectEndpointOfMenu : function(node) {
		this.connectEndpoint(node.id + '-noinput-out',
				node.data.noinput.nextNode + '-in')
		this.connectEndpoint(node.id + '-nomatch-out',
				node.data.nomatch.nextNode + '-in')

		var me = this

		node.data.choices.forEach(function(item) {
			me.connectEndpoint(node.id + '-key-' + item.key + '-out',
					item.nextNode + '-in')
		})
	}
}
...
function dropNode(template, position) {
	...
	/*
	 * $('#positionl').val(position.left);
		$('#positiont').val(position.top);
...
function main() {
	...
	// DataProcess.inputData(data.nodeList)
	 ...
	 $.ajax({
	    type: "post",
	    url: "/test/jsPlumbTest",    //向后端请求数据的url
	    data: {"flowid":flowid},
	    success: function (result) {
	    	result = $.parseJSON( result ) ;
	    	$("#maxNodesn").val(result.maxNodesn);
	    	DataDraw.draw(result);
	    }
	});
}
...
function connectionBeforeDropCheck(info) {
	...
	$.each(jsPlumb.getConnections(), function (idx, connection) {
		...
		if(info.sourceId==sourceId && info.targetId == targetId){ 
			value=false;
		}
    });
	return value;
